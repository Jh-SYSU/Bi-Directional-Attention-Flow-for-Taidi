...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        N, M, JX, JQ, VW, W = \
            ...
            config.max_ques_size, config.word_vocab_size, config.max_word_size
    ...
    def _build_loss(self):
        ...
        #     self.logits, tf.cast(tf.reshape(self.y, [-1, M]), 'float'))
        losses = tf.nn.sigmoid_cross_entropy_with_logits(
            self.logits, tf.cast(tf.reshape(self.y, [N, M]), 'float'))
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        N, M, JX, JQ, VW, d, W = \
            config.batch_size, config.max_num_sents, config.max_sent_size, \
            config.max_ques_size, config.word_vocab_size, config.hidden_size, config.max_word_size
        feed_dict = {}
