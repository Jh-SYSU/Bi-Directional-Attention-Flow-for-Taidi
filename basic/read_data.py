...
def read_data(config, data_type, ref, data_filter=None):
    ...
    word2idx = {}
    idx2vec = []
    for key,vector in shared['word2vec'].items():              
        if len(vector) == 100:
            word2idx[key] = indx
            indx += 1
            idx2vec.append(vector)
    shared['word2vec'] = idx2vec

    x = data['x']
    for i in range(len(x)):
        for j in range(len(x[i])):
            for k in range(len(x[i][j])):
                if(x[i][j][k] in word2idx.keys()):
                    data['x'][i][j][k] = word2idx[x[i][j][k]]
                else:
                    data['x'][i][j][k] = word2idx['-NULL-']
    ...
    for i in range(len(q)):
        for j in range(len(q[i])):
            if(q[i][j] in word2idx.keys()):
                data['q'][i][j] = word2idx[q[i][j]]
            else:
                data['q'][i][j] = word2idx['-NULL-']

    data_set = DataSet(data, data_type, shared=shared, valid_idxs=valid_idxs)
    print('preprocess ready')

    with open('preprod_train.json','w') as f:
...
def update_config(config, data_sets):
    ...
    for data_set in data_sets:
        ...
        for para in data['x']:
            config.max_para_size = max(config.max_para_size, sum(map(len, para)))
            config.max_num_sents = max(config.max_num_sents, len(para))
            config.max_sent_size = max(config.max_sent_size, max(map(len, para)))            
        ...
        for ques in data['q']:
            config.max_ques_size = max(config.max_ques_size, len(ques))
    ...
    config.word_emb_size = len(data_sets[0].shared['word2vec'][0])
    config.word_vocab_size = len(data_sets[0].shared['word2vec'])
