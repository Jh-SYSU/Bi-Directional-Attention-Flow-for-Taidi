...
from basic.evaluator import ForwardEvaluator, MultiGPUF1Evaluator,myMultiGPUF1Evaluator
...
def main(config):
    ...
    with tf.device(config.device):
        print('using',config.device,'to', config.mode)
...
def _train(config):
    ...
    evaluator = myMultiGPUF1Evaluator(config, models, tensor_dict=model.tensor_dict if config.vis else None)
...
def _test(config):
    ...
    word2vec_dict = test_data.shared['word2vec']
    config.emb_mat = np.array(word2vec_dict, dtype = np.float32)

    #pprint(config.__flags, indent=2)
    models = get_multi_gpu_models(config)
    model = models[0]
    evaluator = myMultiGPUF1Evaluator(config, models, tensor_dict=models[0].tensor_dict if config.vis else None)
...
def _predict(config):
    ...
    models = get_multi_gpu_models(config)
    model = models[0]
    predictor = myMultiGPUF1Predictor(config, models, tensor_dict=models[0].tensor_dict)
    graph_handler = GraphHandler(config, model)

    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True))
    graph_handler.initialize(sess)
    num_steps = math.ceil(predict_data.num_examples / (config.batch_size * config.num_gpus))
    if 0 < config.test_num_batches < num_steps:
        num_steps = config.test_num_batches

    labelout = {}
    for multi_batch in tqdm(predict_data.get_multi_batches(config.batch_size, config.num_gpus, num_steps=num_steps, cluster=config.cluster), total=num_steps):
