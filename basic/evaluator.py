...
class MultiGPUF1Evaluator(F1Evaluator):
    def __init__(self, config, models, tensor_dict=None):
        ...
        with tf.name_scope("eval_concat"):
            ...
            self.yp = tf.concat(0, [padded_reshape(model.yp, [N, M]) for model in models])
...
class myMultiGPUF1Evaluator(F1Evaluator):
    def __init__(self, config, models, tensor_dict=None):
        super(myMultiGPUF1Evaluator, self).__init__(config, models[0], tensor_dict=tensor_dict)
        self.models = models
        with tf.name_scope("eval_concat"):
            N, M, JX = config.batch_size, config.max_num_sents, config.max_sent_size
            # self.yp = tf.concat(0, [padded_reshape(model.yp, [N, M]) for model in models])
    ...
    def get_evaluation(self, sess, batch):
        ...
        for i in len(y):
            for j in len(y[i]):
                ...
                if(yp[i][j] == 1):
                    eitherTrue += 1
                if((y[i][j] == 1) and (yp[i][j] == 1)):
                    bothTrue += 1

        self.acc = hitlabel * 1.0 / totallabel
        self.f1 = bothTrue * 2.0 / eitherTrue

        return "{} step {}: accuracy={:.4f}, f1={:.4f}, loss={:.4f}".format(self.data_type, self.global_step, self.acc, self.f1, self.loss)

class myMultiGPUF1Predictor(F1Evaluator):
    def __init__(self, config, models, tensor_dict=None):
        super(myMultiGPUF1Evaluator, self).__init__(config, models[0], tensor_dict=tensor_dict)
        self.models = models
        with tf.name_scope("eval_concat"):
    ...
    def get_evaluation(self, sess, batch, labelout):
        idxs, data_set = self._split_batch(batch)
        assert isinstance(data_set, DataSet)
        feed_dict = self._get_feed_dict(batch)
        global_step, yp, loss, vals = sess.run([self.global_step, self.yp, self.loss, list(self.tensor_dict.values())], feed_dict=feed_dict)
        ...
        yp = yp[:data_set.num_examples]
        idset = data_set.data['idx']
        
        for i in len(yp):
            for j in len(yp[i]):
                if(idset[i][j] in labelout.keys()):
                    pass
                else:
                    labelout[idset[i][j]] = yp[i][j]
