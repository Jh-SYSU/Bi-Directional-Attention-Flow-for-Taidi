...
# sigmoid
def sigmoid_logits(args, size, bias, bias_start=0.0, scope=None, mask=None, wd=0.0, input_keep_prob=1.0, is_train=None):
    with tf.variable_scope(scope or "Sigmoid_Logits"):
        logits = linear(args, size[1], bias, bias_start=bias_start, scope=scope, squeeze=False, wd=wd, input_keep_prob=input_keep_prob,
           is_train=is_train)
        logits = tf.reshape(logits,size)
        return logits
...
def get_logits(args, size, bias, bias_start=0.0, scope=None, mask=None, wd=0.0, input_keep_prob=1.0, is_train=None, func=None):
    ...
    if func == 'sum':
        ...
        return sigmoid_logits(args, size, bias, bias_start=bias_start, scope=scope, mask=mask, wd=wd, input_keep_prob=input_keep_prob, is_train=is_train)
